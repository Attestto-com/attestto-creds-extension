 ...
 *   - `handle`     — `(payload, ctx) => Response`; the real `Response<T>` return type is Story 1.2, `ctx` is Story 1.4.
...
/** Placeholder response — the real `Response<T>` envelope is Story 1.2. */
export type ResponsePlaceholder = unknown
...
/** Handler over the validated payload + injected ctx. Tightened in Stories 1.2/1.4. */
export type RouteHandler<K extends MessageType> = (
  payload: MessagePayload<K>,
  ctx: CtxPlaceholder,
) => Promise<ResponsePlaceholder>
